    ...
    public func topMarkerSetup() {
        safeExecuteBlock({
            func createMarker() {
                ...
                layer.frame = CGRect(x: 0, y: 0, width: UIScreen.main.bounds.width/*self.frame.width*/, height: height)
        ...
        for dotMarker in dotMarkers {
            if let calendarView = calendarView, let dotMarker = dotMarker {
                ...
                if shouldMove && !coloring {
                    ...
                    // TODO: dot animation false로 줬을때 dot color method가 계속 타서 죽음 그래서 주석처리
//                    colorMarker()
                }
                dotIndex += 1
            }
...
//        dayLabel?.frame = bounds
        // TODO: dayLabel 위치 조정
        // TODO: dayView의 bounds를 위치로 조정해주는데, dayView는 x의 좌표가 계속 달라짐. 근데 여기서 bounds를 잡으면 x: 0, y: 0, width: bounds.width, height: bounds.height로 잡힘. 내가 알고 있는 frame과 bounds의 개념이 다른가봄.. 추후 찾아볼 것.
        dayLabel?.frame = CGRect(x: 0, y: -3, width: bounds.width, height: bounds.height)
        
        print("dayLabel11111111-----")
        print(dayLabel?.frame)
